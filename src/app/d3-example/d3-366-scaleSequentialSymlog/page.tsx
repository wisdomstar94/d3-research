"use client"
import { interpolateYlGn, scaleSequentialSymlog, select } from "d3";
...
const Index: NextPage = () => {
  return (
    <>
      <Head>
        <title>d3-366-scaleSequentialSymlog</title>
        <meta name="description" content="d3-366-scaleSequentialSymlog 예시 코드 페이지입니다." />
...
const PageContents = () => {
  ...
  useEffect(() => {
    ...
    /*
      d3.scaleSequentialSymlog 함수는 대칭 로그 변환이 있는 순차 척도로, 대칭 로그 척도와 유사합니다.

      첫 번째 제네릭은 스케일의 기초가 되는 보간기의 출력의 데이터 유형에 해당한다. 
      두 번째 제네릭은 알 수 없는 값의 데이터 유형에 해당한다.
    */
    const fn = scaleSequentialSymlog(interpolateYlGn);
