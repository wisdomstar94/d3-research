"use client"
import { interpolateYlGn, scaleDivergingPow, select } from "d3";
...
const Index: NextPage = () => {
  return (
    <>
      <Head>
        <title>d3-348-scaleDivergingPow</title>
        <meta name="description" content="d3-348-scaleDivergingPow 예시 코드 페이지입니다." />
...
const PageContents = () => {
  ...
  useEffect(() => {
    ...
    /*
      d3.scaleDivergingPow 함수는 지수 변환이 있는 발산 척도로, 검정력 척도와 유사합니다.

      제1 제네릭은 인터폴레이터 리턴 타입의 데이터 타입에 대응한다. 
      두 번째 제네릭은 알 수 없는 값의 데이터 유형에 해당한다.
    */
    const fn = scaleDivergingPow(interpolateYlGn);
