"use client"
import { arc, ascending, chord, ribbonArrow, select } from "d3";
...
const Index: NextPage = () => {
  return (
    <>
      <Head>
        <title>d3-342-ribbonArrow</title>
        <meta name="description" content="d3-342-ribbonArrow 예시 코드 페이지입니다." />
...
const PageContents = () => {
  ...
  useEffect(() => {
    ...
    /*
      d3.ribbonArrow 함수는 기본 설정으로 새 화살표 리본 생성기를 만듭니다.
    */
    ...
    */
    ...
    // 바깥에 arc 를 그리는 코드
    svg
      .datum(chords) // svg 밑의 g 요소(첫번째 요소)에 chord 값(배열)이 바인딩 됨.
    ...
    svg
      .datum(chords) // svg 요소(첫번째 요소)에 chord 값(배열)이 바인딩 됨.
      ...
      .attr("d", ribbonArrow().radius(150) as any) // 각각의 path 요소에 attr 을 지정한다.
